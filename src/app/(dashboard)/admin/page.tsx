...
export default function AdminPage() {
  ...
  async function handleAssign(userId: string) {
    ...
    try {
      ...
      if (payload.status === "success") {
        setFeedback({ kind: "success", message: `${user.email} asignado correctamente` });
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        {loading ? (
          ...
          <div className="mx-auto max-w-4xl space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-wa-border bg-wa-header p-4">
                <div className="flex items-center gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#53bdeb]/10 text-[#53bdeb]">
                    <UsersIcon className="h-5 w-5" />
                  </span>
                  <div>
                    <p className="text-2xl font-bold text-wa-text">{users.length}</p>
                    <p className="text-xs text-wa-text-secondary">Usuarios registrados</p>
                  </div>
                </div>
              </div>
              <div className="rounded-xl border border-wa-border bg-wa-header p-4">
                <div className="flex items-center gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#00a884]/10 text-[#00a884]">
                    <ShieldIcon className="h-5 w-5" />
                  </span>
                  <div>
                    <p className="text-2xl font-bold text-wa-text">{instances.length}</p>
                    <p className="text-xs text-wa-text-secondary">Instancias creadas</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {instances.length > 0 ? (
              <div className="rounded-xl border border-wa-border bg-wa-header p-4">
                <h3 className="mb-3 text-sm font-semibold text-wa-text">Seleccionar instancia</h3>
                <select
              ...
              <div className="rounded-xl border border-wa-border bg-wa-header p-6 text-center">
                <ShieldIcon className="mx-auto h-12 w-12 text-wa-text-secondary/20" />
                <p className="mt-2 text-sm text-wa-text-secondary">No hay instancias creadas</p>
                <p className="text-xs text-wa-text-secondary/60">Crea una en Configuración primero</p>
              </div>
            )}
                  ...
                  <div className="flex flex-col gap-2">
                    {users.map((user) => {
                      ...
                      return (
                        <div
                          key={user.id}
                          className="flex items-center justify-between rounded-lg border border-wa-border/50 px-3 py-2.5"
                        >
                          <div className="flex items-center gap-3">
                            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#53bdeb]/10 text-[#53bdeb] text-xs font-bold">
                              {user.email?.[0]?.toUpperCase() || "?"}
                            </span>
                            <div>
                              <p className="text-sm text-wa-text">{user.email}</p>
                              <p className="text-[10px] text-wa-text-secondary/50">
                                {user.full_name || "Sin nombre"}
                                {isAdmin && (
                                  <span className="ml-2 text-[#00a884]">• Admin</span>
                                )}
                              </p>
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            className={`min-w-[90px] rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                              isAdmin
