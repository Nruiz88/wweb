...
export async function POST(request: Request) {
  ...
  if (!appUrl || appUrl.startsWith("http://localhost") || appUrl.startsWith("https://localhost")) {
    return NextResponse.json({ status: "error", error: "APP_URL no configurado o entorno local" }, { status: 400 });
  }
  ...
  for (const inst of instances) {
    const result = await setWebhook(
      inst.evolution_api_url, inst.evolution_api_key, inst.instance_name, webhookUrl, events, headers
    );
    results.push({ instance_name: inst.instance_name, ok: result.ok, message: result.ok ? undefined : result.message });
  }
