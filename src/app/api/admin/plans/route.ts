...
// GET: Distribucion de planes + detalle por usuario (admin only)
export async function GET() {
  ...
  // Todos los perfiles (con rol, para distinguir admins)
  ...
  const usersWithPlans = (profiles ?? []).map((p) => {
    ...
    const addons = addonsByUser.get(p.id) ?? 0;
    const effectiveMax = base + addons;
